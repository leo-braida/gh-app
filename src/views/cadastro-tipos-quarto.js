...
function CadastroTipoQuarto() {
    ...
    const [tarifaBalcao, setTarifaBalcao] = useState(0);
    ...
    function inicializar() {
        if (idParam == null) {
            ...
            setSelectedCamas(processarDadosDaApi(dados.tipoCamaNoQuarto));
            setTarifaBalcao(dados.tarifaBalcao);
        } 
    }

    async function salvar() {
        const camasParaEnviar = Object.keys(selectedCamas).map(key => ({
            idTipoDeCama: parseInt(key, 10),
            quantidade: selectedCamas[key].quantidade,
        }));

        const precoParaEnviar = preco ? parseFloat(preco.replace('R$ ', '').replace(',', '.')) : 0;
        const tarifaBalcaoParaEnviar = tarifaBalcao ? parseFloat(tarifaBalcao?.replace('R$ ', '').replace(',', '.')) : 0;

        let data = {
            id,
            tipo,
            quantidadeTotal,
            preco: precoParaEnviar,
            tarifaBalcao: tarifaBalcaoParaEnviar,
            tipoCamaNoQuarto: camasParaEnviar,
    ...
    const processarDadosDaApi = (dados) => {
        ...
        return dados.reduce((acc, mapa) => {
            acc[mapa.idTipoDeCama] = { quantidade: mapa.quantidade };
            return acc;
        }, {});
    };
    /*
                ...
                <div className='row'>
                    <div className='col-lg-12'>
                        <div className='bs-component'>
                            ...
                            <FormGroup label={<strong> Tarifa balcão:</strong>} htmlFor='inputTarifaBalcao'>
                                <input
                                    type='text'
                                    id='inputTarifaBalcao'
                                    value={tarifaBalcao}
                                    className='form-control'
                                    name='tarifaBalcao'
                                    onChange={(e) => {
                                    ...
                                    <div key={dado.id} className="flex items-center gap-2">
                                        ...
                                        {selectedCamas[dado.id] !== undefined && (
                                            <input
                                                ...
                                                onChange={(e) => handleQuantidadeChange(dado.id, parseInt(e.target.value, 10), setSelectedCamas)}
